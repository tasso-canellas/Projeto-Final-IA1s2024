...
def train_model(net, X_train, y_train, X_test, y_test, criterion, optimizer, num_epochs, save_adr):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                if epoch_loss < best_val_loss:
                    ...
                    fp = (all_preds[all_labels == 0] != 0).sum().item()
                    tn = (all_preds[all_labels == 0] == 0).sum().item()
                    fn = (all_preds[all_labels == 1] != 1).sum().item()
                    tp = (all_preds[all_labels == 1] == 1).sum().item()
    ...
    return best_epoch, accuracy_list, loss_list, accuracy_train_list, loss_train_list, fpr, fnr
...
i = 1
cross_saves = ['best_model_cross_inception1.pth.tar','best_model_cross_inception2.pth.tar','best_model_cross_inception3.pth.tar','best_model_cross_inception4.pth.tar','best_model_cross_inception5.pth.tar']
...
for train_index, val_index in kf.split(x):
    ...
    best_epoch, accuracy_list, loss_list, accuracy_train_list, loss_train_list, fpr, fnr = train_model(model, X_train, y_train, X_val, y_val, criterion, optimizer, num_epochs=200, save_adr = cross_saves[i-1])
    
    dataframe = []
    
    for epoca in range(len(accuracy_train_list)):
        dictt = {'Validação': i, 'Melhor Época': best_epoch, 'Época': epoca, 'Acurácia de Treino': accuracy_train_list[epoca], 'Loss de Treino':loss_train_list[epoca], 'Acurácia de Teste':accuracy_list[epoca], 'Loss de teste':loss_list[epoca], 'FPR': fpr, 'FNR': fnr }
        dataframe.append(dictt)
    dataframe = pd.DataFrame(dataframe)
    df = pd.concat([df, dataframe], ignore_index=True)
    df.to_csv('/home/tasso/facenet/facenetpytorch/facenet-pytorch/vgg_16/vgg_log.csv', index=False)
